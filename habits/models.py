...
class UserHabit(models.Model):
    ...
    is_pleasant = models.BooleanField(
        default=False, help_text="Является ли привычка приятной"
    )
    is_public = models.BooleanField(
        default=False, help_text="Является ли привычка публичной"
    )
    reminder_time = models.PositiveIntegerField(
        default=60,  # по умолчанию 60 минут
        help_text="Через сколько минут отправить напоминание о привычке",
    )
    ...
    def clean(self):
        ...
        if self.periodicity > 7:
            raise ValidationError(
                "Периодичность привычки должна быть не реже одного раза в 7 дней."
            )

        # Нельзя одновременно выбрать связанную привычку и указать
        # вознаграждение
        if self.reward and self.linked_habit:
            raise ValidationError(
                "Нельзя одновременно указать связанную привычку "
                "и вознаграждение."
            )
        ...
        # Приятной привычке нельзя назначить связанную привычку или
        # вознаграждение
        if self.is_pleasant and (self.linked_habit or self.reward):
            raise ValidationError(
                "Приятной привычке нельзя назначить связанную привычку "
                "или вознаграждение."
            )
